...
export default function Blog() {
  ...
  // Using static sample posts until backend/API is added
  const filteredPosts = activeCategory === "all" 
    ? samplePosts
    : samplePosts.filter((post: BlogPost) => post.category === activeCategory);
        ...
        {filteredPosts.map((post: BlogPost) => (
                  <Card key={post.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                    <div className="relative">
                      <div className="w-full h-48 bg-gradient-to-br from-accent/20 to-accent/40 flex items-center justify-center text-5xl">
                        🎸
                      </div>
                      <Badge 
